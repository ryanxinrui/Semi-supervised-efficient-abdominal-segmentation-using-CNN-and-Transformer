    ...
    def forward(self, x):
        ...
        if self.do_ds:
            for i in range(len(out)):
                # print(self.final[-(i+1)](out[i]))
                # print('--------------------------')
                seg_outputs.append(self.final[-(i+1)](out[i]))

                # seg_outputs.append(self.final_nonlin(self.final[-(i+1)](out[i])))
            # print('--------------------------')
            # print(np.array(seg_outputs[::-1]).shape)
          
            return seg_outputs[::-1]
        else:
            seg_outputs.append(self.final[0](out[-1]))
            # seg_outputs.append(self.final_nonlin(self.final[0](out[-1])))
...
def initialize_nnformer(threeD=True, num_classes=14):
    """
    ...
    network = nnFormer(crop_size=default_dict["crop_size"],
                       embedding_dim=default_dict['embedding_dim'],
                       input_channels=default_dict['input_channels'],
                       num_classes=default_dict['num_classes'],
                       conv_op=conv_op,
                       depths=default_dict['depths'],
                       num_heads=default_dict['num_heads'],
                       patch_size=default_dict['embedding_patch_size'],
                       window_size=default_dict['window_size'],
                       deep_supervision=False)


    print("nnFormer have {} paramerters in total".format(
        sum(x.numel() for x in network.parameters())))
    return network.cuda()

    # if self.load_pretrain_weight:
    #     checkpoint = torch.load('/home/xychen/jsguo/weight/gelunorm_former_skip_global_shift.model',
    #                             map_location='cpu')
...
# input = torch.FloatTensor(1, 1, 128, 256, 256)
# input_var = input.cuda()
# model = initialize_nnformer(threeD=True)
# out = model(input_var)
# print(out.size())